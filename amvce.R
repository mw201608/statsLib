...
#
amvce<-function(y,X,A,tolerance=1.0e-3,maxIter=100,verbose=FALSE){
...
#Author: Minghui Wang <m.h.wang@live.com>
	...
	Xr=X[!iRemoval,]
	nIter=0
	if(verbose==TRUE) cat('Running REML algorithm...\nIter.\tlogL\tV(u)\tV(e)\n')
	loglik0=NA
	loglik=NA
	V=NA
	K3=K %*% solve(K1 %*% K,K1)
	...
	while(nIter<=maxIter){
		...
		if(verbose==TRUE) cat(nIter,'\t',sprintf("%.4f",loglik),'\t',paste(sprintf("%.3f",VC),collapse='\t'),'\n',sep='')
	...
	apVb=solve(crossprod(X,Vi) %*% X)
	b=tcrossprod(apVb,X) %*% (Vi %*% y)[,1]
	res=list(coefficients=b,apVb=apVb,VC=VC,loglik=loglik)
	return(res)
}
